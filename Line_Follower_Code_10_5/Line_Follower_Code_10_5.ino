...
// Variables and Libaries for Motor
  ...
  // Variables for Potentiometer
    ...
    const int P_pin = A2; //proportional control
    const int I_pin = A1; //integral control
...
{
       SpRead = map(analogRead(S_pin),0,1023,15,60); Sp=SpRead;
       kPRead = map(analogRead(P_pin),0,1023,0,60);
       kIRead = 0;
       //kIRead = map(analogRead(I_pin),0,1023,0,5);
       kDRead = map(analogRead(D_pin),0,1023,0,70);
